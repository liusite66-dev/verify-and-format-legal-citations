...
解析入口支持常见外部著录体例，尤其是 GB/T 7714 的 `[J]` 期刊、`[M]` 专著、`[D]` 学位论文及带 DOI 的变体。先按作者、题名、载体、年份、卷期、页码、DOI 拆字段，再按本手册模板改写；只有字段确实无法从原文确定时才报告“信息不完整”。
...
当规则解析无法唯一识别字段时，先由 Agent 在当前引注文本范围内进行语义字段映射，不联网、不补写原文没有的信息。模型可识别作者、题名、载体、出版社/学校、年份、卷期、页码、DOI、案号等字段后，调用 `citation_core.apply_model_fields` 交给确定性模板格式化。字段齐全即可排版；模型辅助条目不得因“规则未覆盖”而一律保留原文。

模型辅助条目必须在报告中标记：解析方式为“模型辅助解析”、解析置信度、模型识别的字段和“未核验书目信息真实性”的待人工提示。置信度不足或字段仍不完整时保持原文。模型不得根据记忆补齐作者、题名、年份、期号或 DOI。
